...
def test_set_animation_breathe()
  ...
  expect("breathe min_brightness", anim.min_brightness, 0)
  expect("breathe max_brightness (100+32)", anim.max_brightness, 132)
  expect("breathe period", anim.period, 3000)
end

def test_set_animation_max_brightness_caps_at_255()
  var lc = setup()
  var anim = lc.set_animation({'rgb': '00FF00', 'hue': 120, 'sat': 255, 'bri': 250, 'power': true}, 'breathe')
  expect("max_brightness capped at 255", anim.max_brightness, 255)
end
...
# ── update_breathe_brightness ──────────────────────────────
def test_update_breathe_brightness()
  var lc = setup()
  var anim = lc.set_animation({'rgb': 'FF0000', 'hue': 0, 'sat': 255, 'bri': 100, 'power': true}, 'breathe')
  lc.update_breathe_brightness(anim, 200)
  expect("min_brightness updated", anim.min_brightness, 0)
  expect("max_brightness updated (200+32)", anim.max_brightness, 232)
end

def test_update_breathe_ignores_non_breathe()
  var lc = setup()
  var anim = lc.set_animation({'rgb': 'FF0000', 'hue': 0, 'sat': 255, 'bri': 100, 'power': true}, 'comet')
  # Should not crash even though comet doesn't have min/max brightness
  lc.update_breathe_brightness(anim, 200)
  expect("comet still has nil tail_length untouched", anim.tail_length, 3)
end
...
def test_add_light_change_rules_uses_delay()
  ...
  # Rules NOT registered yet (waiting for 750ms timer)
...
def run_all()
  ...
  test_set_animation_max_brightness_caps_at_255()
